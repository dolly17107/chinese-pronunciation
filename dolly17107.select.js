export class Dolly17107Select extends HTMLElement {
    select(node) {
        this.querySelectorAll(":scope > [data-select]").forEach(node => { node.removeAttribute("data-selected"); });
...
export class Dolly17107Option extends HTMLElement {
    constructor() {
        super();
        this.addEventListener("click", function(e) { this.parentNode.select(this); }); } }
